...
This plot details the total processing duration (Load + Save duration in seconds) across language runtimes. TRX reduces file sizes (e.g., 1.15 GB for `f16`) while maintaining low processing times (3.4 s - 6.7 s across all languages). In contrast, Python TRK execution falls into a massive outlier segment, requiring up to 105.03 s of total processing time due to unbuffered Python stream iteration overhead. Despite TRX `f16` being ~2× smaller than TRX `f32`, the I/O is not that much faster (likely due to casting operations).
...
### Compression tests
These tests validate that all four language implementations can correctly read and write **compressed** TRX archives (ZIP DEFLATED) without data loss. A compressed file is relayed through every language sequentially, and the final output is compared against the original memory buffer (using an exact tolerance of `1e-4` mm). This guarantees that the deflate/inflate cycle introduces no silent bit corruption and confirms production-readiness, as most real-world TRX files are compressed.

### Edge-case tests
These tests generate synthetic datasets to validate the resilience, scalability, and stability of the parsers under stress. The edge cases evaluated include:
...
*   **Precision overheads:** In compiled languages (C++ and Rust), TRX load times scale closely with data precision sizes (`float16`, `float32`, `float64`), heavily bounded by physical disk I/O. In Python, however, the I/O throughput (MB/s) for `float32` is significantly higher than for `float16`. Because `float32` data maps perfectly to NumPy's native memory architecture for zero-copy reads, the Python parser achieves highly optimized read speeds. This increased throughput largely cancels out the storage advantage of `float16`, resulting in final load times that are relatively close despite `f32` files being twice as large.
...
*   **Checksum overhead for TRX:** TRX architecture naturally favors loading over saving. Loading a TRX file requires zero parsing; the parser reads the centralized ZIP dictionary and instantly accesses binary coordinate arrays via O(1) offsets. However, saving a TRX file is severely CPU-bound because the ZIP specification mandates a Cyclic Redundancy Check (CRC-32) checksum. Even when compression is disabled (store-only), computing this hash over gigabytes of coordinate data heavily bottlenecks the write speed. This explains the major differences between TRX and VTK (for saving only), despite both formats using an approach based on large binary buffers and offsets.
